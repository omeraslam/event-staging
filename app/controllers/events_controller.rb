...
def show
  ...
  def create
    ...
    @eventHTML = '<h1>' + @event.name  + '</h1>'
    @eventHTML += (@event.date_start.blank? ? '' : '<p class="lead"> Join us on ' + @event.date_start.to_date.strftime("%B %d ") + '<p>')
    @eventHTML += '<span class="btn btn-reg open-registration"> Register now</span>'
